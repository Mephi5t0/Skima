        ...
        public async void GenerateEventOfSubscription()
        {
            ...
            foreach (var entry in entries)
            {
                if (entry.CreatedAt.CompareTo(dateOfLastCheckedEntry) > 0)
                {
                    ...
                    var activityId = await entryRepository.GetActivityId(entry.ActivityId);
                    var maraphoneId = await activityRepository.GetMaraphoneIdAsync(activityId);
                    var maraphone = await maraphoneRepository.GetAsync(maraphoneId, CancellationToken.None);
